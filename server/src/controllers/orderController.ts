...
export default class OrderController extends BaseController {
  ...
  delete = async (req: Request, res: Response) => {
    try {
      ...
      if (userIsAdmin) {
        const formatedDeleteEntity: OrderSendMail = {
          code: model.code,
  ...
  private sendMailDeleteOrderToUser = async (order: OrderSendMail) => {
    try {
      const htmlString = `
      <div
      ...
      await sendMail(order.userEmail, 'Hủy đơn hàng', htmlString)
    } catch (error) {
      console.log(error)
    }
  }
